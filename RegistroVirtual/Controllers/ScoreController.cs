    ...
    public class ScoreController : Controller
    {
        ...
        public ActionResult LoadScores(string selectedClass, string selectedYear, string selectedTrimester)
        {
            ...
            try
            {
                ...
                columns.Add(new WebGridColumn() { ColumnName = "StudentName", Header = "Estudiante", Format = (item) => { return new HtmlString(string.Format("<label id='StudentName'>{0}</label> <input type=hidden id='StudentId' value={1} /> <input type=hidden id='RegisterProfileId' value={2} />", item.StudentName, item.StudentId, selectedRegisterProfile.Id)); }, Style = "col3Width" });
                ...
                foreach (ExamModel exam in selectedRegisterProfile.Exams)
                {
                    ...
                    columns.Add(new WebGridColumn() {
                        ...
                        Format = (item) => { return new HtmlString(string.Format("<input type='number' class='exam-score' min=0 max=100 id={0} value={1} data-exam-id={2} />", examScoreFieldId, 
                            currentScores.SingleOrDefault(x => x.StudentId.Equals(item.StudentId) && x.ExamResults.Where(t => t.ExamId.Equals(exam.Id)).Count() > 0) != null ? currentScores.SingleOrDefault(x => x.StudentId.Equals(item.StudentId)).ExamResults.Where(t => t.ExamId.Equals(exam.Id)).First().ExamScore : 0, exam.Id)); },
                        Style = "col1Width" });

                    columns.Add(new WebGridColumn()
                    {
                        Header = string.Format("{0} - {1}pts", exam.Name, exam.Percentage),
                        Format = (item) => { return new HtmlString(string.Format("<input type='number' min=0 class='exam-points {0}' value={1} max={2} data-exam-id={3} />", examPointsFieldId, 0, exam.Score, exam.Id)); },
                        Style = "col1Width"
                    });

                    columns.Add(new WebGridColumn() {
                        Header = string.Format("{0} - {1}%", exam.Name, exam.Percentage),
                        Format = (item) => { return new HtmlString(string.Format("<input type='number' min=0 class='exam-percentage {0}' value={1} max={2} data-exam-id={3} readonly />", examPercentageFieldId, 0, exam.Percentage, exam.Id )); },
                        Style = "col1Width" });
                ...
                foreach (ExtraclassWorkModel extraclass in selectedRegisterProfile.ExtraclassWorks)
                {
                    ...
                    columns.Add(new WebGridColumn() {
                        ...
                        Format = (item) => { return new HtmlString(string.Format("<input type='number' class='extraclass-score' min=0 max={0} id={1} value={2} data-extraclass-id={3} />", extraclass.Percentage, extraclassPercentageFieldId,
                            currentScores.SingleOrDefault(x => x.StudentId.Equals(item.StudentId) && x.ExtraclasWorkResults.Where(t => t.ExtraclassWorkId.Equals(extraclass.Id)).Count() > 0) != null ? currentScores.SingleOrDefault(x => x.StudentId.Equals(item.StudentId)).ExtraclasWorkResults.Where(t => t.ExtraclassWorkId.Equals(extraclass.Id)).First().ExtraclassWorkPercentage : 0, extraclass.Id)); },
                        Style = "col1Width" });
